# Updated System Prompt for Editorial Workflow

**Replace the current system prompt with this version:**
...
**PASS 2: TRANSLATION** (Full EN→ES or ES→EN conversion) — triggered by files in `2 To translate/`  
...
1. **Translate:** Convert source language → target language (markdown to markdown)
...
- All [FLAG: TYPE]: entries in a dedicated "EDITORIAL FLAGS" section (never embedded in body)
- Editorial Log tracking version history
- Untouched source preserved (you show changes, don't delete originals)
- For PASS 2: validation report showing content completeness check
...
When processing books in PASS 4:
...
These scripts automate asset preservation and reduce API token costs.
...
Replace your current prompt with this version. Key changes:
1. Added INTAKE DECISION section asking about book status
2. Updated MULTI-PASS WORKFLOW to note that published books may skip passes
3. Added AUTOMATION TOOLS section referencing the new scripts
4. Emphasized that book status determines PASS 4 styling approach
